...
export async function fetchDataHotel(e: { slug: string, checkIn: string, checkOut: string }): Promise<HotelData.full> {
  const response = await fetch(
...
export async function fetchOrderSession(e: {
  "hotelId": string;
  "startDate": string;
  "endDate": string;
  "roomData": {
    ...
    "room_name": string;
    "total_price": string;
    "by_person": string; //Зарим өрөөнүүд хүнээр захиалга үүсгэдэг. Энэ үед room_number 0 by_person хүний тоо байна
  }[];
}): Promise<orderSession> {
  try {
    console.log(e.roomData, 'thisone')
    const response = await fetch(`${process.env.WEB_URL}/ihotel/order`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        "order_hotelid": e.hotelId,
        "order_roomdata": e.roomData,
        "order_startdate": e.startDate,
        "order_enddate": e.endDate,
      }),
    });
    ...
    const response = await fetch(
      `${process.env.WEB_URL}/ihotel/order/card/create`,
      {
        ...
        body: JSON.stringify({
          ...
          "beneficiary_account_number": e.beneficiary_name,
        }),
      },
    );
